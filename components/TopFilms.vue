<template>
  <div>
    ...
    >
      <div 
        class="relative w-[150px] h-[250px] gap-4 group cursor-pointer" 
...
const items = ref<{ id: number, imageUrl: string, title: string, genres: string[], description: string }[]>([]);
...
onMounted(async () => {
  ...
  if (movies.value) {
    items.value = movies.value.results.slice(0, 12).map(movie => {
      return {
        id: movie.id, // Adicione o ID do filme
        imageUrl: `https://image.tmdb.org/t/p/w500${movie.poster_path}`,
        title: movie.title,
        description: truncateText(movie.overview, 120), // Trunca a descrição para 120 caracteres
